...
const CommentSection = ({
  ...
  const displayComments = showAll
    ? sortedComments
    : sortedComments.slice(0, maxPreview);

  const handleAddComment = (text: string) => {
    onAddComment?.(postId, text, replyingTo?.parentId);
    setReplyingTo(null);
  };

  const handleReply = (username: string, commentId: string) => {
    setReplyingTo({ username, commentId, parentId: commentId });
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between text-sm">
        <span className="text-muted-foreground">댓글 {comments.length}개</span>
